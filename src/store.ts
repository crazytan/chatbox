...
import { type Settings, createSession, type Session, type Message, type Config } from "./types";
...
export async function writeSettings(settings: Settings) {
  ...
  await api.writeStore("settings", settings);
}
...
export async function readSessions(settings: Settings): Promise<Session[]> {
  const sessions: Session[] | undefined = await api.readStore("chat-sessions");
  if (sessions == null) {
...
export async function writeSessions(sessions: Session[]) {
  await api.writeStore("chat-sessions", sessions);
}
...
export default function useStore() {
  ...
  useEffect(() => {
    ...
    return () => {
      if (updateCheckTimer.current != null) {
  ...
  const [currentSession, switchCurrentSession] = useState<Session>(
    chatSessions[0]
  );
  useEffect(() => {
  ...
  const [toasts, _setToasts] = useState<Array<{ id: string; content: string }>>([]);
